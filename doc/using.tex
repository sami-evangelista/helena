...
\item\optionDef{D}{delta}\\ This option can activate the delta
  ...
  also slow the search. Option \shortForm{K} can be set to influence
  this.
\item\optionDef{K=N}{k-delta=N}\\ This is the parameter of the
...
\item\optionDef{S}{state-caching}\\ The principle of state caching is
  ...
  states.
\item\optionDef{s}{cache-size=N}\\ When using state caching Helena
