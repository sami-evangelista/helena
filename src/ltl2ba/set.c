...
void spin_print_set(FILE * f, int *pos, int *neg) /* prints the content of a set for spin */
{
  int i, j, start = 1;
...
void C_print_set(FILE * f, int *pos, int *neg)
{
  int i, j, start = 1;
  for(i = 0; i < sym_size; i++) 
    for(j = 0; j < mod; j++) {
	...
	if(!start)
	  fprintf(f, " && ");
	fprintf(f, "state_proposition_%s (s)", sym_table[mod * i + j]);
	start = 0;
      }
      if(neg[i] & (1 << j)) {
	if(!start)
